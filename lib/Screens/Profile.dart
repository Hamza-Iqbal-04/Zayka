...
class _ProfileScreenState extends State<ProfileScreen>
  ...
  Widget _buildLanguageSwitchItem(
      Map<String, dynamic> item, LanguageProvider provider) {
    return Container(
...
class _EditProfileScreenState extends State<EditProfileScreen> {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text(AppStrings.get('profile_updated', context))));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(AppStrings.get('edit_profile', context),
            ...
            children: [
              ...
              _buildTextField(
                ...
                validator: (value) => (value == null || value.isEmpty)
                    ? AppStrings.get('enter_name_error', context)
                    : null,
              ),
              const SizedBox(height: 20),
              _buildTextField(
                ...
                validator: (value) => (value == null || value.isEmpty)
                    ? AppStrings.get('enter_phone_error', context)
                    : null,
              ),
            ],
          ),
      ...
      child: SizedBox(
        ...
        child: ElevatedButton(
          ...
          child: _isLoading
              ...
              : Text(AppStrings.get('save_changes', context),
                  style: const TextStyle(
...
class _SavedAddressesScreenState extends State<SavedAddressesScreen> {
  ...
  Future<void> _updateAddresses(
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text('Error updating addresses: ${e.toString()}')));
      }
    }
  }

  Future<void> _setAsDefault(
      int index, List<Map<String, dynamic>> addresses) async {
    final updatedAddresses =
        addresses.map((addr) => {...addr, 'isDefault': false}).toList();
                ...
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      Text(
                        isEditing
                            ? AppStrings.get('edit_address', context)
                            : AppStrings.get('add_new_address', context),
                        style: const TextStyle(
                          ...
                          children: [
                            TileLayer(
                              ...
                              userAgentPackageName:
                                  'com.example.mitra_da_dhaba',
                            ),
                            if (selectedGeoPoint != null)
                              MarkerLayer(
                                markers: [
                                  Marker(
                                    point: LatLng(selectedGeoPoint!.latitude,
                      ...
                      TextField(
                        ...
                        decoration: InputDecoration(
                          hintText:
                              AppStrings.get('search_location_hint', context),
                          filled: true,
                          fillColor: Colors.grey[
                              100], // Slightly different to differentiate from fields
                      ...
                      DropdownButtonFormField<String>(
                        ...
                        items:
                            [...standardLabels, 'Custom'].map((String value) {
                      ...
                      TextField(
                        ...
                        decoration: InputDecoration(
                          ...
                          prefixIcon:
                              Icon(Icons.add_road, color: Colors.grey[600]),
                          filled: true,
                          fillColor: Colors.white,
                          floatingLabelStyle:
                              const TextStyle(color: AppColors.primaryBlue),
                        ...
                        children: [
                          Expanded(
                            child: TextField(
                              ...
                              decoration: InputDecoration(
                                labelText:
                                    AppStrings.get('floor_label', context),
                                prefixIcon:
                          ...
                          Expanded(
                            child: TextField(
                              ...
                              decoration: InputDecoration(
                                labelText:
                                    AppStrings.get('flat_label', context),
                      ...
                      SizedBox(
                        ...
                        child: ElevatedButton(
                          onPressed: () async {
                            ...
                            if (labelController.text.trim().isEmpty) {
                              ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(
                                      content: Text(
                                          "Please provide a label for this address")));
                              return;
                            }
                            ...
                            final newAddress = {
                              ...
                              'isDefault':
                                  isEditing ? address!['isDefault'] : false,
                              'geolocation':
                                  selectedGeoPoint ?? const GeoPoint(0, 0),
                            };
                          ...
                          child: Text(
                            isEditing
                                ? AppStrings.get('save_changes', context)
                                : AppStrings.get('add_new_address', context),
                            style: const TextStyle(
  ...
  Future<void> _reverseGeocode(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        flatController.text =
            addressDetails['house_number'] ?? addressDetails['flat'] ?? '';
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data.isNotEmpty) {
          ...
          mapController.move(
              point, 15.0); // Increased zoom to 15 for better view

          // Auto-fill the address details for this location
          await _reverseGeocode(lat, lon, streetController, cityController,
              buildingController, floorController, flatController);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: Colors.grey[100],
      appBar: AppBar(
        title: Text(AppStrings.get('saved_addresses', context),
            style: const TextStyle(fontWeight: FontWeight.bold)),
        backgroundColor: Colors.grey[100],
        elevation: 0,
        foregroundColor: Colors.black,
      ...
      body: user == null || AuthUtils.getDocId(user) == 'guest'
          ...
          : StreamBuilder<DocumentSnapshot>(
              ...
              builder: (context, snapshot) {
                ...
                final addresses =
                    (data['address'] as List?)?.cast<Map<String, dynamic>>() ??
                        [];
        ...
        builder: (context) {
          ...
          return StreamBuilder<DocumentSnapshot>(
            ...
            builder: (context, snapshot) {
              if (!snapshot.hasData || !snapshot.data!.exists) {
                return FloatingActionButton.extended(
                  onPressed: () => _showAddEditAddressSheet(
                      allAddresses: <Map<String, dynamic>>[]),
              ...
              return FloatingActionButton.extended(
                onPressed: () =>
                    _showAddEditAddressSheet(allAddresses: addresses),
                label: Text(AppStrings.get('add_new_address', context)),
                icon: const Icon(Icons.add),
                backgroundColor: AppColors.primaryBlue,
                foregroundColor: Colors.white,
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 20),
          Text(AppStrings.get('no_addresses_title', context),
              style:
                  const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
          const SizedBox(height: 8),
          Text(AppStrings.get('no_addresses_subtitle', context),
        ...
        child: Column(
          ...
          children: [
              ...
              children: [
                ...
                if (isDefault)
                  Container(
                    ...
                    child: Text(AppStrings.get('default_badge', context),
                        style: const TextStyle(
                            color: Colors.green,
                            fontSize: 11,
                            fontWeight: FontWeight.bold)),
            ...
            Text(
              ...
              style:
                  TextStyle(color: Colors.grey[700], fontSize: 15, height: 1.4),
            ),
            const SizedBox(height: 12),
            Row(
...
class _FavoritesScreenState extends State<FavoritesScreen> {
  ...
  Future<void> _removeFavorite(String itemId) async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text(AppStrings.get('removed_from_favorites', context)),
  ...
  void _addItemToCart(Map<String, dynamic> itemData, String itemId) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
          content: Text(
              '$displayName ${AppStrings.get('items_added_to_cart', context)}'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: Colors.grey[100],
      appBar: AppBar(
        title: Text(AppStrings.get('my_favorites', context),
            style: const TextStyle(fontWeight: FontWeight.bold)),
        backgroundColor: Colors.grey[100],
        elevation: 0,
        foregroundColor: Colors.black,
      ...
      body: AuthUtils.getDocId(user) == 'guest'
          ...
          : StreamBuilder<DocumentSnapshot>(
              ...
              builder: (context, snapshot) {
                ...
                final data = snapshot.data!.data() as Map<String, dynamic>;
                final favorites =
                    (data['favorites'] as Map<String, dynamic>?) ?? {};
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 20),
          Text(AppStrings.get('no_favorites_title', context),
              style:
                  const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
          const SizedBox(height: 8),
          Padding(
                ...
                children: [
                  Row(
                    children: [
                      ...
                      const SizedBox(width: 12),
                      Text(AppStrings.get('view_cart', context),
                          style: const TextStyle(
                              color: Colors.white,
                              fontWeight: FontWeight.bold,
                              fontSize: 16)),
...
class HelpSupportScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: Colors.grey[100],
      appBar: AppBar(
        title: Text(AppStrings.get('help_support', context),
            style: const TextStyle(fontWeight: FontWeight.bold)),
        backgroundColor: Colors.grey[100],
        elevation: 0,
        foregroundColor: Colors.black,
